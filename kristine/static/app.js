...
d3.json("../county/"+countyName, function(err,data){
  if (err) throw err;
  ...
  //  console.log("whitesData",whitesData)
    ...
    function findMinAndMax(dataColumn) {
      yMin = d3.min(data, function(data) {
        ...
        return +data[dataColumn] * .8; //make it a little less so the first data point is not at the bottom
        // return (data.AsianOver200 < data.BlackOver200) ? (data.AsianOver200 * .8) : (data.BlackOver200 * .8);
      });
  
      yMax = d3.max(data, function(data) {
        return +data[dataColumn] * 1.1;  // make it a little more so the last data point is not at the top
...
// aianDataFiltered = testData.filter(function (d){
//   return d.race=='AIAN';
// });
    ...
    var lineGen = d3.line()
    .x(function(d) {
      return xLinearScale(d.Year);
    })
    .y(function(d) {
      return yLinearScale(d.TotalPopulation);
    });

    aianDataFiltered = data.filter(function (d){
      return d.Race=='AIAN';
    });
    
    console.log("aianDataFiltered", aianDataFiltered)

    aian.append('svg:path')
    .attr('d', lineGen(aianDataFiltered))
    .attr('stroke', 'green')
    .attr('stroke-width', 2)
    .attr('fill', 'none');

    blackDataFiltered = data.filter(function (d){
      return d.Race=='Black';
    });

    aian.append('svg:path')
    .attr('d', lineGen(blackDataFiltered))
    .attr('stroke', 'red')
    .attr('stroke-width', 2)
    .attr('fill', 'none');

    hispanicDataFiltered = data.filter(function (d){
      return d.Race=='Hispanic';
    });

    aian.append('svg:path')
    .attr('d', lineGen(hispanicDataFiltered))
    .attr('stroke', 'blue')
    .attr('stroke-width', 2)
    .attr('fill', 'none');
...
// use this same datset
// the total population chart------------------------------------------------------------------------------------------------
var totalSvg = dimple.newSvg(".totalpop", 690, 400);
d3.json("/county/"+ countyName, function (data) {
  ...
  // Create the chart
  var myChart = new dimple.chart(totalSvg, data);
  ...
  y = myChart.addMeasureAxis("y", "TotalPopulation");
  myChart.addSeries(["Year","TotalPopulation", "Race"], dimple.plot.bubble);
  // myChart.addSeries("TotalPopulation", dimple.plot.bubble);
  var myLegend = myChart.addLegend(570, 75, 100, 400, "Right");
  myChart.draw();
  ...
  myLegend.shapes.selectAll("rect")
    ...
    .on("click", function (e) {
      ...
      // Filter the data
      myChart.data = dimple.filterData(data, "Race", filterValues);
